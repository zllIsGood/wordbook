 ...
 * @Last Modified time: 2020-05-27 19:43:23
 */
class ScoreService {
    /**记录当前奖励类型*/
    ...
    public static Reward(awardType, fun?) {
        ...
        this.init()
        let token = 'token:' + AndroidJs.getToken()
        let obj = {
    ...
    public static init() {
        ...
        if (!window['AndroidJs']) {
            ...
            wx.showToast({ title: 'AndroidJs 不存在' })
            let AndroidJs = {
                getToken: () => {
                    // return 'token:572701782376062976'
                    return '572701782376062976'
                }
            }
            window['AndroidJs'] = AndroidJs
        }
        else if (!window['AndroidJs'].getToken) {
            wx.showToast({ title: 'AndroidJs.getToken() 不存在' })
        }
    }
}
