 ...
 * @Last Modified time: 2020-06-20 16:14:43
 */
if (!window['wx']) {
    window['wx'] = {
        login: async function () {
            return RequestUtil.postPromise({
                ...
                if (res.code === 0 && StringUtil.isNotBlank(res.data.token)) {
                    ...
                    if (loginRes.gameData.dailyRecruitCount != null) {
                        HeroModel.ins().dailyRecruitCount = loginRes.gameData.energyConfig.dailyRecruitCount - loginRes.gameData.dailyRecruitCount
                    }
                    // App.ins().setOpenAdConfig(loginRes.gameData.adConfig)
